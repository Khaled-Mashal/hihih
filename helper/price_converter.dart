...
class PriceConverter {
    ...
    static String convertPriceV2(double? price, {double? discount, String? discountType, bool forDM = false}) {
    if(Get.find<SplashController>().module!=null&& Get.find<SplashController>().module!.themeId==3&&price==0){
      return "";
    }
    if(price==0){
      return "multiple_choices".tr;
    }
  ...
  static Widget convertAnimationPrice(double? price, {double? discount, String? discountType, bool forDM = false, TextStyle? textStyle}) {
    ...
    static Widget convertAnimationPriceV2(double? price, {double? discount, String? discountType, bool forDM = false, TextStyle? textStyle}) {
      if(discount != null && discountType != null){
      if(discountType == 'amount') {
        price = price! - discount;
      }else if(discountType == 'percent') {
    ...
    return Directionality(
      ...
      child: AnimatedFlipCounter(
        duration: const Duration(milliseconds: 500),
        value: toFixed(price! < 0.0?0:price!),
        textStyle: textStyle ?? robotoMedium,
        fractionDigits: forDM ? 0 : Get.find<SplashController>().configModel!.digitAfterDecimalPoint!,
        prefix: isRightSide ? '' : Get.find<SplashController>().configModel!.currencySymbol!,
        suffix: isRightSide ? Get.find<SplashController>().configModel!.currencySymbol! : '',
