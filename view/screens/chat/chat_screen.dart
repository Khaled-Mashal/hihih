...
class ChatScreen extends StatefulWidget {
  ...
  bool isClinic;
  ChatScreen({Key? key, required this.notificationBody, required this.user, this.conversationID, this.index, this.fromNotification = false, this.isClinic = false}) : super(key: key);
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    super.initState();
    print("object::: widget.user2 ${widget.user}");
    initCall();
  }

  void initCall() {
    if (Get.find<AuthController>().isLoggedIn()) {
      if (widget.notificationBody != null && widget.notificationBody!.isClinic != null && widget.notificationBody!.isClinic == '1') {
        widget.isClinic = true;
      }
      print("isClinic ${widget.isClinic}");
      if (widget.isClinic) {
        Get.find<ChatController>().getMessages(1, widget.notificationBody, widget.user, widget.conversationID, firstLoad: true, isClinic: widget.isClinic);
      } else {
                                        ...
                                        ? SingleChildScrollView(
                                            ...
                                            child: PaginatedListView(
                                              ...
                                              onPaginate: (int? offset) async {
                                                if (widget.isClinic) {
                                                  await chatController.getMessages(offset!, widget.notificationBody, widget.user, widget.conversationID, isClinic: widget.isClinic);
                                                } else {
                                                              ...
                                                              children: [
                                                                ...
                                                                Positioned(
                                                                  ...
                                                                  child: InkWell(
                                                                    ...
                                                                    child: Container(
                                                                      decoration:
                                                                          const BoxDecoration(color: Colors.white, borderRadius: BorderRadius.all(Radius.circular(Dimensions.paddingSizeDefault))),
                                                                      child: const Padding(
