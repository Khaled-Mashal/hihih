...
class ChatDoctorScreen extends StatefulWidget {
  ...
  final User? user;
  late ConversationDoctor? conversationDoctor;
  final int? conversationID;
  final int? index;
  final bool fromNotification;
   ChatDoctorScreen({Key? key, required this.notificationBody, required this.user,this.conversationDoctor ,this.conversationID, this.index, this.fromNotification = false}) : super(key: key);
...
class _ChatDoctorScreenState extends State<ChatDoctorScreen> {
  ...
  void initState() {
    super.initState();
// print("object::: widget.notificationBody ${widget.notificationBody}");
// print("object::: widget.conversationDoctor ${widget.conversationDoctor}");
    initCall();
...
// print("object::: widget.notificationBody ${widget.notificationBody}");
// print("object::: widget.user ${widget.user}");
// print("object::: widget.conversationID ${widget.conversationID}");

    if(Get.find<AuthController>().isLoggedIn()) {
      // Get.find<ChatController>().getConversationList(1, type: ResponsiveHelper.isDesktop(Get.context) ? 'vendor' : '');
      if(widget.conversationDoctor==null &&widget.notificationBody!=null){
          widget.conversationDoctor= ConversationDoctor(key:widget.notificationBody!.senderType,name:widget.notificationBody!.sectionName??'',logo:widget.notificationBody!.sectionImage??'',id: int.parse(widget.notificationBody!.sectionId??'0') );
      }
      Get.find<ChatController>().getMessages(1, widget.notificationBody, widget.user, widget.conversationID,type: widget.conversationDoctor!.key!, firstLoad: true,isDoctor: true);
       ...
       WillPopScope(
        ...
        child: Scaffold(
          ...
          appBar: (ResponsiveHelper.isDesktop(context) ? const WebMenuBar() : AppBar(
            ...
            title:Text(widget.conversationDoctor!.name??''),
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: ClipOval(child: CustomImage(
                    image:'$baseUrl'
                        '/${widget.conversationDoctor!.logo??''}',
                    // image:'$baseUrl'
                    ...
                    return Expanded(child: chatController.messageModel != null ? chatController.messageModel!.messages!.isNotEmpty ? SingleChildScrollView(
                      ...
                      child: PaginatedListView(
                        ...
                        onPaginate: (int? offset) async => await chatController.getMessages(
                          offset!, widget.notificationBody, widget.user, widget.conversationID,type: widget.conversationDoctor!.key! ,isDoctor: true
                        ),
                            ...
                            onTap: () async {//1001
                              if(chatController.isSendButtonActive) {
                                await chatController.sendMessage(
                                  ...
                                  isDoctor:true,
                                  type: widget.conversationDoctor!.key??''
                                );
