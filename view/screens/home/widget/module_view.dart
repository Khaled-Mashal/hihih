...
class ModuleView extends StatelessWidget {
  ...
  final isService;
  List<ModuleModel> moduleListProducts = [];
  List<ModuleModel> moduleListServices = [];

  ModuleView({Key? key, required this.splashController, this.isService = false})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    if (splashController.moduleList != null &&
      ...
      moduleListProducts = splashController.moduleList!
          .where((element) => element.themeId != 3)
          .toList();
      moduleListServices = splashController.moduleList!
          .where((element) => element.themeId == 3)
          .toList();
    }
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      splashController.moduleList != null
          ? splashController.moduleList!.isNotEmpty
              ? Column(
                  children: [
                    if (moduleListProducts.isNotEmpty)
                      GridView.builder(
                        gridDelegate:
                        ...
                        itemCount: moduleListProducts.length,
                        shrinkWrap: true,
                        physics: const NeverScrollableScrollPhysics(),
                        itemBuilder: (context, index) {
                          return InkWell(
                            onTap: () => splashController.switchModule2(
                                moduleListProducts[index], true),
                            child: Container(
                              ...
                              child: Column(
                                  ...
                                  children: [
                                    ClipRRect(
                                      ...
                                      child: CustomImage(
                                        image:
                                            '${splashController.configModel!.baseUrls!.moduleImageUrl}/${moduleListProducts[index].icon}',
                                        height: 65,
                                        width: 65,
                                      ),
                                    ),
                                    ...
                                    Center(
                                        child: Text(
                                      moduleListProducts[index].moduleName!,
                    ...
                    if (moduleListServices.isNotEmpty)
                      Padding(
                        padding: const EdgeInsets.fromLTRB(10, 6, 10, 2),
                        child: TitleWidget(title: 'services'.tr),
                      ),
                    GridView.builder(
                      gridDelegate:
                          const SliverGridDelegateWithFixedCrossAxisCount(
                      ...
                      padding:
                          const EdgeInsets.all(Dimensions.paddingSizeSmall),
                      // itemCount: splashController.moduleList!.length,
                      itemCount: moduleListServices.length,
                      shrinkWrap: true,
                      physics: const NeverScrollableScrollPhysics(),
                      itemBuilder: (context, index) {
                        return InkWell(
                          onTap: () => splashController.switchModule2(
                              moduleListServices[index], true),
                          child: Container(
                            decoration: BoxDecoration(
                              borderRadius: BorderRadius.circular(
                                  Dimensions.radiusDefault),
                            ...
                            child: Column(
                                ...
                                children: [
                                  ClipRRect(
                                    ...
                                    child: CustomImage(
                                      image:
                                          '${splashController.configModel!.baseUrls!.moduleImageUrl}/${moduleListServices[index].icon}',
                                      height: 65,
                                      width: 65,
                                    ),
                                  ),
                                  ...
                                  Center(
                                      child: Text(
                                    moduleListServices[index].moduleName!,
                                    textAlign: TextAlign.center,
                                    maxLines: 2,
                                    overflow: TextOverflow.ellipsis,
                                    style: robotoMedium.copyWith(
