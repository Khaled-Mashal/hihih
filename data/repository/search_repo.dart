...
class SearchRepo {
  ...
  Future<Response> getSearchQRData(String? query, bool isStore) async {
    return await apiClient.getData('${AppConstants.searchUri}${isStore ? 'stores' : 'items'}/search?barcode_no=$query&offset=1&limit=50');
  }
  ...
  Future<bool> saveSearchQRHistory(List<String> searchHistories) async {
    return await sharedPreferences.setStringList(AppConstants.searchQRHistory, searchHistories);
  }

  List<String> getSearchQRAddress() {
    return sharedPreferences.getStringList(AppConstants.searchQRHistory) ?? [];
  }

  Future<bool> clearSearchQRHistory() async {
    return sharedPreferences.setStringList(AppConstants.searchQRHistory, []);
  }
}
