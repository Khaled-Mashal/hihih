...
class SplashController extends GetxController implements GetxService {
  ...
  Future<bool> updateConfigData(
      {bool loadModuleData = false,
      bool loadLandingData = false,
      selfReload = false}) async {
    dynamic dd = await splashRepo.updateConfigData();
...
//herrrrrrr1001
  Module getModuleConfig2(String? moduleType) {
    Module module = Module.fromJson(_data!['module_config'][moduleType]);
    // if (moduleType == 'food') {
    module.newVariation = true;
    // } else {
  ...
  Future<void> getModules(
      {Map<String, String>? headers, bool selfReload = false}) async {
    _moduleIndex = 0;
    Response response = await splashRepo.getModules(headers: headers);
    if (response.statusCode == 200) {
      _moduleList = [];
    ...
    update();
    if (!selfReload) {
      await splashRepo.updateModules();
      await getModules(selfReload: true);
    }
  }
  ...
  void switchModule2(ModuleModel changeTomodule, bool fromPhone) async {
    if (_module == null || _module!.id != changeTomodule.id) {
      await Get.find<SplashController>().setModule(changeTomodule);
      Get.find<CartController>().getCartData();
      HomeScreen.loadData(true);
    }
  }
