...
class BannerController extends GetxController implements GetxService {
  ...
  Future<void> getFeaturedBanner({bool selfReload = false}) async {
    ...
    update();
    if (!selfReload) {
      await bannerRepo.updateFeaturedBannerList();
      await getFeaturedBanner(selfReload: true);
    }
  }

  Future<void> getBannerList(bool reload, {bool selfReload = false}) async {
    if(_bannerImageList == null || reload) {
      _bannerImageList = null;
      int? moduleID = Get.find<SplashController>().module?.id;
      if(moduleID!=null){
           Response response = await bannerRepo.getBannerList(moduleID.toString());
      if (response.statusCode == 200) {
        _bannerImageList = [];
        _bannerDataList = [];
        BannerModel bannerModel = BannerModel.fromJson(response.body);
    ...
    update();
      if (!selfReload) {
        await bannerRepo.updateBannerList(moduleID.toString());
        await getBannerList(reload, selfReload: true);
      }
