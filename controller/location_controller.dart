...
class LocationController extends GetxController implements GetxService {
  ...
  Future<ZoneResponseModel> getZone(String? lat, String? long, bool markerLoad, {bool updateInAddress = false, bool selfReload = false}) async {
    if (markerLoad) {
      _loading = true;
    } else {
      _isLoading = true;
    ...
    update();
    if (!selfReload) {
      await locationRepo.updateZone(lat, long);
      await getZone(lat, long, markerLoad, selfReload: true);
    }
  ...
  Future<void> getAddressList({bool selfReload = false}) async {
    ...
    update();
    if (!selfReload) {
      await locationRepo.updateAllAddress();
      await getAddressList(selfReload: true);
    }
  }
  ...
  Future<AddressModel> setLocation(String? placeID, String? address, GoogleMapController? mapController) async {
    ...
    _pickPosition = Position(
        latitude: latLng.latitude, longitude: latLng.longitude, timestamp: DateTime.now(), accuracy: 1, altitude: 1, heading: 1, speed: 1, speedAccuracy: 1, altitudeAccuracy: 0, headingAccuracy: 0);
        ...
        if (ResponsiveHelper.isDesktop(Get.context)) {
          showGeneralDialog(
              ...
              pageBuilder: (_, __, ___) {
                return SizedBox(
                    height: 300, width: 300, child: PickMapScreen(fromSignUp: (page == RouteHelper.signUp), canRoute: false, fromAddAddress: false, route: null, googleMapController: mapController));
              });
        } else {
