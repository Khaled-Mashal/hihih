...
class ChatController extends GetxController implements GetxService {
  ...
  ConversationsDoctorModel? _conversationDoctorModel;
  ...
  ConversationsDoctorModel? get conversationDoctorModel =>
      _conversationDoctorModel;
  ...
  Future<void> getConversationDoctorList(int offset, {String type = ''}) async {
    _hasAdmin = true;
    _searchConversationModel = null;
    Response response = await chatRepo.getConversationDoctorList();
    if (response.statusCode == 200) {
      _conversationDoctorModel =
          ConversationsDoctorModel.fromJson(response.body);
      // _conversationDoctorModel!.conversations!.addAll(ConversationsDoctorModel.fromJson(response.body).conversations!);
    } else {
  ...
  Future<void> getMessages(
    ...
    if (isDoctor && type != null) {
      response =
          await chatRepo.getMessages(offset, 0, type, null, isDoctor: isDoctor);
    ...
    if (response != null &&
      ...
      // print("SSSSSSSSSSSSectoion $isDoctor");
      ApiChecker.checkApi(response!);
    }
    update();
  }
  ...
  Future<Response?> sendMessage(
    ...
    if (isDoctor) {
      response = await chatRepo.sendMessage(message, myImages, 0, type!, null);
    } else if (notificationBody == null || notificationBody.adminId != null) {
    ...
    if (response!.statusCode == 200) {
      ...
      Future.delayed(const Duration(seconds: 2), () {
        getMessages(1, notificationBody, null, conversationID, isDoctor: true);
      });
    }
    update();
    return response;
