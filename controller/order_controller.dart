...
class OrderController extends GetxController implements GetxService {
  ...
  Future<void> getRunningOrders(int offset, {bool isUpdate = false,bool selfReload = false}) async {
    if(offset == 1) {
    ...
    if (response.statusCode == 200) {
      ...
      update();
      if (!selfReload) {
        await orderRepo.updateRunningOrderList(offset);
        await getRunningOrders(offset, selfReload: true);
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<void> getHistoryOrders(int offset, {bool isUpdate = false,bool selfReload = false}) async {
    if(offset == 1) {
    ...
    if (response.statusCode == 200) {
      ...
      update();
      if (!selfReload) {
        await orderRepo.updateHistoryOrderList(offset);
        await getHistoryOrders(offset, selfReload: true);
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<List<OrderDetailsModel>?> getOrderDetails(String orderID,{bool selfReload = false}) async {
    _orderDetails = null;
    _isLoading = true;
    ...
    update();
    if (!selfReload) {
      await orderRepo.updateOrderDetails(orderID);
      await getOrderDetails(orderID, selfReload: true);
    }
  ...
  Future<ResponseModel?> trackOrder(String? orderID, OrderModel? orderModel, bool fromTracking, {bool selfReload = false}) async {
    _trackModel = null;
    _responseModel = null;
    if(!fromTracking) {
      _orderDetails = null;
    ...
    if(orderModel == null) {
     if (!selfReload) _isLoading = true;
      ...
      if (!selfReload) _isLoading = false;
      update();//dd
      if (!selfReload) {
        await orderRepo.updateTrackOrder(orderID);
        await trackOrder(orderID, orderModel, fromTracking, selfReload: true);
      }
    }else {
  ...
  Future<ResponseModel?> timerTrackOrder(String orderID, {bool selfReload = false}) async {
    _showCancelled = false;
    ...
    if (response.statusCode == 200) {
      ...
      if (!selfReload) {
        await orderRepo.updateTrackOrder(orderID);
        await timerTrackOrder(orderID,selfReload: true);
      }
