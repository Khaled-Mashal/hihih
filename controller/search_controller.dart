...
class SearchingController extends GetxController implements GetxService {
  ...
  void searchQRData(String? query, bool fromHome) async {
    if ((_isStore && query!.isNotEmpty && query != _storeResultText) || (!_isStore && query!.isNotEmpty && (query != _itemResultText || fromHome))) {
      _searchHomeText = query;
      _searchText = query;
      _rating = -1;
      ...
      print("QQQQRR 5 ${query}");
      Response response = await searchRepo.getSearchQRData(query, _isStore);
      print("QQQQRR 6 ${query}");
      if (response.statusCode == 200) {
        if (query.isEmpty) {
          if (_isStore) {
            _searchStoreList = [];
  ...
  void getHistoryList() {
    ...
    void getQRHistoryList() {
    _isSearchMode = true;
    _searchText = '';
    _historyList = [];
    _historyList.addAll(searchRepo.getSearchQRAddress());
  }
  ...
  void removeHistory(int index) {
    ...
    void removeQRHistory(int index) {
    _historyList.removeAt(index);
    searchRepo.saveSearchQRHistory(_historyList);
    update();
  }
  ...
  void clearSearchQRAddress() async {
    searchRepo.clearSearchQRHistory();
    _historyList = [];
    update();
  }
