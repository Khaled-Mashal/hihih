...
class ItemController extends GetxController implements GetxService {
  ...
  Future<void> getPopularItemList(bool reload, String type, bool notify,
    ...
    if (_popularItemList == null || reload) {
      ...
      if (moduleID != null) {
        Response response =
            await itemRepo.getPopularItemList(type, moduleID.toString());
        ...
        update();
        if (!selfReload) {
          await itemRepo.updatePopularItemList(type, moduleID.toString());
          await getPopularItemList(reload, type, notify, selfReload: true);
        }
      }
    }
  }
  ...
  Future<void> getReviewedItemList(bool reload, String type, bool notify,
    ...
    if (_reviewedItemList == null || reload) {
      ...
      if (moduleID != null) {
        Response response =
            await itemRepo.getReviewedItemList(type, moduleID.toString());
        if (response.statusCode == 200) {
          _reviewedItemList = [];
        ...
        update();
        if (!selfReload) {
          await itemRepo.updateReviewedItemList(type, moduleID.toString());
          await getReviewedItemList(reload, type, notify, selfReload: true);
        }
      }
    }
  }
  ...
  void initData(Item? item, CartModel? cart) {
    ...
    if (cart != null) {
      ...
      ///herrrrrrr1001
      else if (item.foodVariations != null) {
        _selectedVariations.addAll(cart.foodVariations!);
        for (int index = 0; index < item.foodVariations!.length; index++) {
          _collapsVariation.add(true);
        }
  ...
  void setQuantity(bool isIncrement, int? stock, int? quantityLimit,
    ...
    if (isIncrement) {
      if (false /*Get.find<SplashController>().configModel!.moduleConfig!.module!.stock! && _quantity! >= stock!*/) {
  ...
  Future<void> getProductDetails(Item item) async {
    ...
    await itemRepo.updateItemDetails(item.id);
  }
