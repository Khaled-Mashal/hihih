...
class StoreController extends GetxController implements GetxService {
  ...
  CartSuggestItemModel? _cartSuggestItemModel2;
  ...
  CartSuggestItemModel? get cartSuggestItemModel2 => _cartSuggestItemModel2;
  bool get isSearching => _isSearching;

void resetStoreData(){
  print("resetStoreData before  ${_storeItemModel!}");
        // showCustomSnackBar("resetStoreData before  ${_storeItemModel}");
  _store=null;
  _recommendedItemModel=null;
  _storeItemModel=null;
  //  showCustomSnackBar("resetStoreData after  ${_storeItemModel}");
  print("resetStoreData after  ${_storeItemModel}");
}
  ...
  Future<void> getRestaurantRecommendedItemList(int? storeId, bool reload,{bool selfReload = false}) async {
    if(reload) {
      _recommendedItemModel = null;
      // _storeModel = null;
      update();
    ...
    if (response.statusCode == 200) {
      ...
      update();
      if (!selfReload) {
        await storeRepo.updateStoreRecommendedItemList(storeId);
        await getRestaurantRecommendedItemList(storeId,true,selfReload: true);
      }
  }

 Future<void> getCartStoreSuggestedItemList2(int? storeId,int? categoryId) async {
  _cartSuggestItemModel2=null;
    print("${AppConstants.cartStoreSuggestedItemsUri2} call getCartStoreSuggestedItemList2");
    Response response = await storeRepo.getCartStoreSuggestedItemList2(storeId,categoryId);
    if (response.statusCode == 200) {
    print("${AppConstants.cartStoreSuggestedItemsUri2} data ${response.body}");
      _cartSuggestItemModel2 = CartSuggestItemModel.fromJson(response.body);
        print("${AppConstants.cartStoreSuggestedItemsUri2} Data ${_cartSuggestItemModel2!.toJson()}");
    } else {
      ApiChecker.checkApi(response);
    }
    update();
  ...
  Future<void> getStoreList(int offset, bool reload, {bool selfReload = false}) async {
    if(reload) {
      _storeModel = null;
      update();
    }
    ...
    if(moduleID!=null){
    String lat = Get.find<LocationController>().getUserAddress()!.latitude??'';
    String long = Get.find<LocationController>().getUserAddress()!.longitude??'';
    Response response = await storeRepo.getStoreList(offset, _storeType, moduleID.toString(), lat, long);
    if (response.statusCode == 200) {
      if (offset == 1) {
      ...
      update();
      if (!selfReload) {
        await storeRepo.updateStoreList(_storeModel!.offset!, _storeType, moduleID.toString(), lat, long);
        await getStoreList(_storeModel!.offset!,_storeModel==null?reload:false, selfReload: true);
      }
    } else {
  ...
  Future<void> getPopularStoreList(bool reload, String type, bool notify, {bool selfReload = false}) async {
    _type = type;
    if(reload) {
    ...
    //  int moduleID = Get.find<SplashController>().module!=null&& Get.find<SplashController>().module!.id!=null?Get.find<SplashController>().module!.id!:0; 
      int? moduleID = Get.find<SplashController>().module?.id;
    if(moduleID!=null){
      String lat = Get.find<LocationController>().getUserAddress()!.latitude??'';
      String long = Get.find<LocationController>().getUserAddress()!.longitude??'';
      Response response = await storeRepo.getPopularStoreList(type, moduleID.toString(), lat, long);
      if (response.statusCode == 200) {
        _popularStoreList = [];
      ...
      update();
      if (!selfReload) {
        await storeRepo.updatePopularStoreList(type, moduleID.toString());
        await getPopularStoreList(reload, type, true, selfReload: true);
      }
    } 
    }
  }

  Future<void> getLatestStoreList(bool reload, String type, bool notify, {bool selfReload = false}) async {
    _type = type;
    if(reload){
    ...
    if(_latestStoreList == null || reload) {
      ...
      String lat = Get.find<LocationController>().getUserAddress()!.latitude??'';
      String long = Get.find<LocationController>().getUserAddress()!.longitude??'';
      Response response = await storeRepo.getLatestStoreList(type, moduleID.toString(), lat, long);
      if (response.statusCode == 200) {
        _latestStoreList = [];
      ...
      update();
      if(!selfReload) {
        await storeRepo.updateLatestStoreList(type, moduleID.toString());
        await getLatestStoreList(reload, type, notify, selfReload: true);
      }
     }
    }
  }

  Future<void> getFeaturedStoreList({bool selfReload = false}) async {
    String lat = Get.find<LocationController>().getUserAddress()!.latitude??'';
    String long = Get.find<LocationController>().getUserAddress()!.longitude??'';
    Response response = await storeRepo.getFeaturedStoreList(lat, long);
    if (response.statusCode == 200) {
      _featuredStoreList = [];
     ...
     update();
    if (!selfReload) {
      await storeRepo.updateFeaturedStoreList();
      await getFeaturedStoreList(selfReload: true);
    }
  }
  ...
  Future<Store?> getStoreDetails(Store store, bool fromModule, {bool fromCart = false, String slug = '',bool selfReload = false}) async {
    _categoryIndex = 0;
    if(store.name != null) {
      _store = store;
    }else {
      ...
      _isLoading = false;
       update();
      if (!selfReload) {
        await storeRepo.updateStoreDetails(store.id.toString(), fromCart, slug);
        await getStoreDetails(store, fromModule,fromCart:fromCart, slug:slug, selfReload: true);
      }
    }
    return _store;
  }

  Future<void> getStoreItemList(int? storeID, int offset, String type, bool notify,{bool selfReload = false}) async {
    ...
    if (response.statusCode == 200) {
      ...
      ApiChecker.checkApi(response);
        ...
        update();
    if (!selfReload) {
      await storeRepo.updateStoreItemList(
        storeID,
        offset,
        (_store != null &&  _store!.categoryIds!.isNotEmpty  && _categoryIndex != 0) 
      ...
      await getStoreItemList(storeID, offset, type, notify, selfReload: true);
    }
  }
